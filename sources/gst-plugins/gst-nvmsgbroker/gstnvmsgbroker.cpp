    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame; l_frame = l_frame->next) {
      ...
      for (l = user_meta_list; l; l = l->next) {
        ...
        if (user_meta && user_meta->base_meta.meta_type == NVDS_PAYLOAD_META) {
          ...
          if(self->newConnHandle && NvMsgBrokerHandleMap[self->newConnHandle]->connection_alive == true) {
            ...
            if (err != NV_MSGBROKER_API_OK) {
              ...
              GST_ERROR_OBJECT (self, "gstmsgbroker send : connection not alive, message not published");

          if(self->newConnHandle && NvMsgBrokerHandleMap[self->newConnHandle]->broker_disconnect == true) {
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    ...
    nvdsgst_msgbroker,
    "Message broker",
    plugin_init, "6.1", "Proprietary", "NvMsgBroker", "http://nvidia.com")
