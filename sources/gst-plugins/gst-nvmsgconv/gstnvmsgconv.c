...
gst_nvmsgconv_start (GstBaseTransform * trans)
{
  ...
  if (self->msg2pLib != NULL) {
     ...
     self->msg2p_generate_new = (nvds_msg2p_generate_ptr_new) dlsym (self->libHandle, "nvds_msg2p_generate_new");
     self->msg2p_generate_multiple_new = (nvds_msg2p_generate_multiple_ptr_new) dlsym (self->libHandle, "nvds_msg2p_generate_multiple_new");
    ...
    self->msg2p_generate_multiple_new = (nvds_msg2p_generate_multiple_ptr_new) nvds_msg2p_generate_multiple_new;
...
static int add_payload_to_frame(GstNvMsgConv *self, NvDsPayload **payloads , guint payloadCount,
NvDsBatchMeta *batch_meta, void *frame_meta, FILE* debug_payload_dump_file){
  for(uint p=0; p < payloadCount; ++p)
    {
      ...
      if (user_payload_meta) {
        ...
        if(self->is_video) {
          ...
          nvds_add_user_meta_to_audio_frame ((NvDsAudioFrameMeta*)frame_meta, user_payload_meta);
        }
        g_atomic_int_inc(&self->numActivePayloads);
      } else {
      ...
      if(self->msg2pNewApi){
        ...
        if (self->payloadType == NVDS_PAYLOAD_DEEPSTREAM_MINIMAL) {
            ...
            if(self->multiplePayloads)
                payloads = self->msg2p_generate_multiple_new (self->pCtx, &meta_info, &payloadCount);
            else
            {
            ...
            int errcode = add_payload_to_frame(self, payloads, payloadCount, batch_meta, frame_meta, debug_payload_dump_file);
          ...
          if(self->is_video) {
            for (NvDsObjectMetaList *obj_l = ((NvDsFrameMeta*)frame_meta)->obj_meta_list; obj_l; obj_l = obj_l->next) {
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    ...
    nvdsgst_msgconv,
    "Metadata conversion",
    plugin_init, "6.1", "Proprietary", "NvMsgConv", "http://nvidia.com")
